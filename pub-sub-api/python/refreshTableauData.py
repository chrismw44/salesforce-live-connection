...
def eventDataRequest(endpoint, queryE, access_token):
    # Make the request
    responseE = requests.post(endpoint, json={'query': queryE}, headers={
...
def eventContactDataRequest(endpoint, queryECon, relationIDs, access_token):
    ...
    # Make the request
    responseC = requests.post(endpoint, json={'query': queryECon, 'variables': {
                              'relationIDs': relationIDs}}, headers={'Authorization': f'Bearer {access_token}'})
...
def eventColleagueDataRequest(endpoint, queryEC, access_token):
    ...
    # Make the request
    responseEC = requests.post(endpoint, json={'query': queryEC}, headers={
                               'Authorization': f'Bearer {access_token}'})
    ...
    if responseEC.status_code == 200:
        ...
        raise Exception(
            "Error gathering Event Colleague data:", responseEC.text)
...
def eventTransform(dfTransformE):
    if dfTransformE is not None:
        if not dfTransformE.empty:
            ...
            dfTransformE['Requested By'] = dfTransformE['Recorder Email'].apply(
                lambda x: x.split('@')[0] if pd.notnull(x) else None)
            dfTransformE['Requested By'] = dfTransformE['Requested By'].apply(
                lambda x: re.sub(r'[^a-zA-Z]', ' ', x) if pd.notnull(x) else None)
            dfTransformE['Requested By'] = dfTransformE['Requested By'].apply(
                lambda x: x.title() if pd.notnull(x) else None)

            # Set Event date columns to datetime
            dfTransformE['Start Date Time'] = pd.to_datetime(
                dfTransformE['Start Date Time'])
            dfTransformE['End Date Time'] = pd.to_datetime(
                dfTransformE['End Date Time'])
            ...
            dfTransformE['Meeting Start Date'] = dfTransformE['Start Date Time'].apply(
                map_date_to_string)
            dfTransformE['Meeting End Date'] = dfTransformE['End Date Time'].apply(
                map_date_to_string)
            dfTransformE['Meeting Dates'] = dfTransformE.apply(lambda row: map_date_range_to_string(
                generate_date_range(row['Start Date Time'], row['End Date Time'])), axis=1)
            ...
            dfTransformE['ProductFieldSplit'] = dfTransformE.loc[:,
                                                                 'Product Field']
            dfTransformE['ContactsSplit'] = dfTransformE.loc[:, 'Contacts']
            dfTransformE['FujifilmAttendeesSplit'] = dfTransformE.loc[:,
                                                                      'Fujifilm Attendees']
            dfTransformE['PartnerAttendeesSplit'] = dfTransformE.loc[:,
                                                                     'Partner Attendees']
            dfTransformE['EventRelationIDsSplit'] = dfTransformE.loc[:,
                                                                     'Event Relation IDs']
            dfTransformE['MeetingDatesSplit'] = dfTransformE.loc[:,
                                                                 'Meeting Dates']
            dfTransformE['CountrySplit'] = dfTransformE.loc[:, 'Country']

            # Split and pivot product and contact fields
            dfTransformE = dfTransformE.assign(
                ProductFieldSplit=dfTransformE.ProductFieldSplit.str.split(", ")).explode("ProductFieldSplit")
            dfTransformE.rename(
                columns={"ProductFieldSplit": "Product Field Split"}, inplace=True)

            dfTransformE = dfTransformE.assign(
                ContactsSplit=dfTransformE.ContactsSplit.str.split(", ")).explode("ContactsSplit")
            dfTransformE.rename(
                columns={"ContactsSplit": "Contacts Split"}, inplace=True)

            dfTransformE = dfTransformE.assign(FujifilmAttendeesSplit=dfTransformE.FujifilmAttendeesSplit.str.split(
                ", ")).explode("FujifilmAttendeesSplit")
            dfTransformE.rename(
                columns={"FujifilmAttendeesSplit": "Fujifilm Attendees Split"}, inplace=True)

            dfTransformE = dfTransformE.assign(PartnerAttendeesSplit=dfTransformE.PartnerAttendeesSplit.str.split(
                ", ")).explode("PartnerAttendeesSplit")
            dfTransformE.rename(
                columns={"PartnerAttendeesSplit": "Partner Attendees Split"}, inplace=True)

            dfTransformE = dfTransformE.assign(EventRelationIDsSplit=dfTransformE.EventRelationIDsSplit.str.split(
                ",")).explode("EventRelationIDsSplit")
            dfTransformE.rename(
                columns={"EventRelationIDsSplit": "Event Relation IDs Split"}, inplace=True)

            dfTransformE = dfTransformE.assign(
                MeetingDatesSplit=dfTransformE.MeetingDatesSplit.str.split(", ")).explode("MeetingDatesSplit")
            dfTransformE.rename(
                columns={"MeetingDatesSplit": "Meeting Dates Split"}, inplace=True)
            dfTransformE['Dates Attending Split'] = dfTransformE.loc[:,
                                                                     'Meeting Dates Split']

            dfTransformE = dfTransformE.assign(
                CountrySplit=dfTransformE.CountrySplit.str.split(", ")).explode("CountrySplit")
            dfTransformE.rename(
                columns={"CountrySplit": "Country Split"}, inplace=True)

            # Create new merged columns for Company, Recorder Email and Requested By
            dfTransformE['Company'] = np.where(
                dfTransformE['Account ID'] == '0016700006EtLL3AAN', dfTransformE['Subject'], dfTransformE['Account Name'])
            dfTransformE['Recorder Email'] = dfTransformE['Recorder Email'].fillna(
                dfTransformE['User Email'])
            dfTransformE['Requested By'] = dfTransformE['Requested By'].fillna(
                dfTransformE['User Name'])
...
def groupContacts(dfTransformE):
    if dfTransformE is not None:
        if not dfTransformE.empty:
            dfGroupedContacts = dfTransformE[[
                'Activity ID', 'Contact Full Name']].dropna()
            dfGroupedContacts = dfGroupedContacts.rename(
                columns={'Contact Full Name': 'SF Contacts'})
            dfGroupedContacts = dfGroupedContacts.groupby(
                "Activity ID", as_index=False).agg(lambda x: ', '.join(set(x.tolist())))
            dfTransformE = pd.merge(
                dfTransformE, dfGroupedContacts, on="Activity ID", how="left")
            dfTransformE['Contacts'] = dfTransformE['Contacts'].fillna(
                dfTransformE['SF Contacts'])
            dfTransformE['Name'] = dfTransformE['Contacts Split'].fillna(
                dfTransformE['Contact Full Name'])
            dfTransformE = dfTransformE.drop(
                ['SF Contacts', 'Contact Full Name'], axis=1)
...
def groupDatesAttending(dfTransformE):
    if dfTransformE is not None:
        if not dfTransformE.empty:
            dfTransformE['NameCompany'] = dfTransformE['Name'] + \
                dfTransformE['Company']
            dfTransformE['NameCompany'] = dfTransformE['NameCompany'].str.replace(
                " ", "")
            dfGroupedDatesAttending = dfTransformE[[
                'NameCompany', 'Dates Attending Split']].dropna()
            dfGroupedDatesAttending = dfGroupedDatesAttending.rename(
                columns={'Dates Attending Split': 'Dates Attending'})
            dfGroupedDatesAttending = dfGroupedDatesAttending.groupby(
                "NameCompany", as_index=False).agg(lambda x: ', '.join(set(x)))
            ...
            def sort_dates(row):
                ...
                date_objs = [datetime.strptime(date.split(
                    ' - ')[0], '%B %d') for date in dates]
                # Sort the datetime objects
                sorted_dates = sorted(date_objs)
                # Convert back to string format
                sorted_dates_str = [date.strftime(
                    '%B %d - %A') for date in sorted_dates]
                # Join the sorted dates
                return ', '.join(sorted_dates_str)

            # Apply the function to each row in the 'dates' column
            dfGroupedDatesAttending['Dates Attending'] = dfGroupedDatesAttending['Dates Attending'].apply(
                sort_dates)
            dfTransformE = pd.merge(
                dfTransformE, dfGroupedDatesAttending, on="NameCompany", how="left")
...
def eventColleagueTransform(dfTransformEC):
    if dfTransformEC is not None:
        if not dfTransformEC.empty:
            ...
            # Clean phone number
            dfTransformEC['Mobile Phone Cleaned'] = dfTransformEC['Mobile Phone'].astype(
                str).fillna('')

            dfTransformEC['Mobile Phone Cleaned'] = dfTransformEC['Mobile Phone Cleaned'].str.replace(
                r'\D', '', regex=True)

            dfTransformEC['Mobile Phone Cleaned'] = dfTransformEC['Mobile Phone Cleaned'].replace(
            ...
            # Duplicate fields for splitting
            dfTransformEC['DatesAttendingSplit'] = dfTransformEC.loc[:,
                                                                     'Dates Attending']
            dfTransformEC['LanguagesSpokenSplit'] = dfTransformEC.loc[:,
                                                                      'Languages Spoken']
            dfTransformEC['ProductSpecialitySplit'] = dfTransformEC.loc[:,
                                                                        'Product Speciality']
            dfTransformEC['Country Split'] = dfTransformEC.loc[:, 'Country']

            # Split and pivot date, language and product fields
            dfTransformEC = dfTransformEC.assign(
                DatesAttendingSplit=dfTransformEC.DatesAttendingSplit.str.split(", ")).explode("DatesAttendingSplit")
            dfTransformEC.rename(
                columns={"DatesAttendingSplit": "Dates Attending Split"}, inplace=True)

            dfTransformEC = dfTransformEC.assign(
                LanguagesSpokenSplit=dfTransformEC.LanguagesSpokenSplit.str.split(", ")).explode("LanguagesSpokenSplit")
            dfTransformEC.rename(
                columns={"LanguagesSpokenSplit": "Languages Spoken Split"}, inplace=True)

            dfTransformEC = dfTransformEC.assign(ProductSpecialitySplit=dfTransformEC.ProductSpecialitySplit.str.split(
                ", ")).explode("ProductSpecialitySplit")
            dfTransformEC.rename(
                columns={"ProductSpecialitySplit": "Product Speciality Split"}, inplace=True)

            # Add company column with the value 'Fujifilm'
            dfTransformEC['Company'] = 'Fujifilm'

            # Create a full name column
            dfTransformEC['Name'] = dfTransformEC['First Name'] + \
                ' ' + dfTransformEC['Last Name']

            dfTransformEC['NameCompany'] = dfTransformEC['Name'] + \
                dfTransformEC['Company']
...
def joinedDataFrame(dfTransformE, dfTransformEC):
    ...
    if dfTransformE.empty and not dfTransformEC.empty:
        ...
        dfAttendanceMeeting['Record ID'] = dfAttendanceMeeting['Record ID_Attendees'].fillna(
            dfAttendanceMeeting['Activity ID'])

    if not dfAttendanceMeeting.empty:
        dfAttendanceMeeting['Languages Spoken'] = dfAttendanceMeeting['Languages Spoken'].fillna(
            dfAttendanceMeeting['Primary Language'])

        dfAttendanceMeeting['Start Date Time'] = dfAttendanceMeeting['Start Date Time'].dt.strftime(
            '%Y-%m-%d %H:%M:%S')
        dfAttendanceMeeting['Start Date Time'] = pd.to_datetime(
            dfAttendanceMeeting['Start Date Time'])
        dfAttendanceMeeting['Start Date Time'] += pd.Timedelta(hours=2)
        dfAttendanceMeeting['End Date Time'] = dfAttendanceMeeting['End Date Time'].dt.strftime(
            '%Y-%m-%d %H:%M:%S')
        dfAttendanceMeeting['End Date Time'] = pd.to_datetime(
            dfAttendanceMeeting['End Date Time'])
        ...
        dfAttendanceMeeting['Company'] = dfAttendanceMeeting['Company'].str.replace(
            "&#39;", "'")
        dfAttendanceMeeting.loc[dfAttendanceMeeting['Objective1'] ==
                                'Drupa Meeting', 'Meeting Room Allocated'] = 'Not Requested'

    for feature in dfAttendanceMeeting:
        if dfAttendanceMeeting[feature].dtype == 'object':
            dfAttendanceMeeting[feature] = dfAttendanceMeeting[feature].astype(
                'string[pyarrow]')
    ...
    with server.auth.sign_in(tableau_auth):
        ...
        try:
            datasource = server.datasources.publish(
                new_datasource, sfDrupaLive,  mode='Overwrite')
            print(
                "5/5: Datasource published. Datasource ID: {0}".format(datasource.id))
            message = "Datasource " + sfDrupaLive + \
                " published. Datasource ID: {0}".format(
                    datasource.id) + " successfull\n"
        except:
...
def process_event_data(ENDPOINT, queryEvent, queryEventContacts, queryEventColleague, ACCESS_TOKEN):
    ...
    dfDataC = eventContactDataRequest(
        ENDPOINT, queryEventContacts, relationIDs, ACCESS_TOKEN)

    # Merge event data with contact data
    dfTransformE = pd.merge(dfTransformE, dfDataC,
                            on="Event Relation IDs Split", how="left")
    ...
    dfDataEC = eventColleagueDataRequest(
        ENDPOINT, queryEventColleague, ACCESS_TOKEN)
    ...
    pantab.frame_to_hyper(dfAttendanceMeeting,
                          sfDrupaLive, table="sfDrupaLive")

    # Publish to server
    publishToServer(TOKEN_NAME, PERSONAL_ACCESS_TOKEN, SITE_ID,
                    SERVER_NAME, PROJECT_NAME, sfDrupaLive)
